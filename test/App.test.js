  ...
  describe("Meme API", () => {
    ...
    describe("GET /api/v1/memes/:id", () => {
      ...
      test("should return a meme by id", async () => {
        ...
        const response = await request(app).get(`/api/v1/memes/${meme._id}`);
        // Verificamos que el status de la respuesta sea 200 (OK)
        expect(response.status).toBe(200);
        // Verificamos que el tipo de contenido sea JSON
        expect(response.type).toBe("application/json");
      ...
      test("should return 404 if meme not found", async () => {
        ...
        const response = await request(app).get(
          "/api/v1/memes/5f50c31e1c12d4a1f8c7e5a1"
        );
    ...
    describe("POST /api/v1/memes", () => {
      ...
      test("should create a new meme", async () => {
        ...
        expect(response.body.ok).toBe(true);
        // Verificamos que el nombre del meme en la respuesta sea correcto
        expect(response.body.body.name).toBe("Homer");
        // Guardamos el ID del meme creado para usarlo en tests posteriores
        createdMemeId = response.body.body._id;
      });
    ...
    afterEach(async () => {
      await memeModel.deleteMany({ name: "Marge" });
    });
...
// El problema principal era que el código estaba utilizando métodos y propiedades específicas de bases de datos SQL (como Sequelize),
// mientras que MongoDB es una base de datos NoSQL. Los cambios principales incluyen:
// 1. Cambiar 'id' por '_id' para los identificadores de MongoDB.
// 2. Usar 'deleteMany' en lugar de 'destroy' para eliminar documentos.
// 3. Ajustar los IDs de ejemplo a un formato válido de MongoDB (ObjectId).
// 4. Eliminar referencias a 'where' en las consultas, ya que MongoDB usa un enfoque diferente para las consultas.
